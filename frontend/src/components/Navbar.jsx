    ...
    <div className='flex items-center justify-between py-5 font-medium mb-8'>
      ...
      <div className='flex items-center gap-3 sm:gap-6'>
        ...
        <div className='group relative'>
          <img onClick={() => token ? null : window.location.href = '/login'} className='w-5 cursor-pointer' src={assets.profile_icon} alt="Profile" />
          {token && (
            <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4 z-50'>
              <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-500 rounded shadow-md'>
                ...
                <p onClick={() => { setToken(''); localStorage.removeItem('token'); window.location.href = '/login'; }} className='cursor-pointer hover:text-black'>Logout</p>
              </div>
            </div>
          )}
        </div>
        ...
        <img onClick={() => setVisible(true)} src={assets.list_icon} className='w-5 cursor-pointer sm:hidden' alt="Menu" />
      </div>
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all z-50 ${visible ? 'w-full' : 'w-0'}`}>
        <div className='flex flex-col text-gray-600'>
          <div onClick={() => setVisible(false)} className='flex items-center gap-2 p-3 cursor-pointer'>
            <img className='h-4 rotate-180' src={assets.right_arrow_icon} alt="Back" />
